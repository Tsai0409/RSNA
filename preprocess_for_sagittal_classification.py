...
oof = pd.concat([pd.read_csv(f'{WORKING_DIR}/results/{config}/oof_fold1.csv') for fold in range(1)])  # sagittal 中 valid 預測出來的 bounding box 資訊
...
df_path = f'{WORKING_DIR}/csv_train/region_estimation_by_yolox_holdout_6/sagittal_train_clear_filtered.csv'  # 去除掉 groundturth(只保留 Sagittal T1、Sagittal T2/STIR) -> train valid
...
p = 'sagittal_spinal_range2_rolling5.csv'  # 不知道為什麼只有 left 的資料 -> train
...
for left_right in ['left', 'right']:
    ...
    p = f'sagittal_{left_right}_nfn_range2_rolling5.csv'
    df.to_csv(p, index=False)
    print(p)
...
for left_right in ['left', 'right']:
    ...
    p = f'sagittal_{left_right}_ss_range2_rolling5.csv'
    df.to_csv(p, index=False)
    print(p)
